...
public sealed class AmbilightInProcessPlayer : IDisposable
{
    ...
    private async Task RunAsync(string sessionId, string binPath, DeviceMapping mapping, double startSeconds, CancellationToken cancellationToken)
    {
        try
        {
            ...
            float outputGammaCfg = (float)cfg.AmbilightOutputGamma;
            float brightnessAdj = ClampF((float)cfg.AmbilightBrightness, 0.0f, 5.0f);
            ...
            while (!cancellationToken.IsCancellationRequested && frameIndex < frames.Count)
            {
                ...
                // Mean frame luminance only feeds the legacy adaptive gamma; skip the pass otherwise.
                float avgLum = 0f;
                if (autoBrightness)
                {
                    float sumLum = 0f;
                ...
                float outputGamma = autoBrightness
                    ? 1.0f / ClampF(gammaBase * (1.0f - (avgLum / 255.0f) * 0.6f), 1.0f, 3.0f)
                    : ClampF(outputGammaCfg, 0.1f, 5.0f);
                ...
                for (int t = 0; t < totalTgt; t++)
                {
                    ...
                    float rF = rG * brightnessAdj * 255.0f;
                    float gF = gG * brightnessAdj * 255.0f;
                    float bF = bG * brightnessAdj * 255.0f;
