...
public sealed class AmbilightInProcessExtractor
{
    ...
    /// ordering are unaffected, so the AMb3 header stays valid.
    /// </summary>
        ...
        bool edgeWeighted)
    {
        ...
        // Default mode: a plain mean in linear light, with no relative rescale. The per-zone luminance
        // rescale below is an amplifier — it can push a bright spot to 3x — so it belongs with the edge
        // weighting it was tuned against, not with picture-extending averaging.
        if (!edgeWeighted)
        {
            ComputeFrameColorsLinearMean(frame, width, height, zones, output);
            return;
        }
    ...
    /// <summary>
    /// Single pass over the zones taking an unweighted mean in linear light, with no relative rescale.
    /// This is the picture-extending behaviour: a dark zone averages dark, and a black frame produces
    /// black LEDs.
    /// </summary>
    private static void ComputeFrameColorsLinearMean(
        byte[] frame,
        int width,
        int height,
        (int x1, int y1, int x2, int y2)[] zones,
